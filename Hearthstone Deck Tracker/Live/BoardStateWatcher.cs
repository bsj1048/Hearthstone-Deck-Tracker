...
namespace Hearthstone_Deck_Tracker.Live
{
	internal class BoardStateWatcher
	{
		...
		private BoardState GetBoardState()
		{
			...
			var playerCardsDict = new Dictionary<int, int[]>();
			if(deck != null)
			{
				var playerCards = player.PlayerCardList.Where(x => !x.IsCreated && !x.Jousted).ToList();
				foreach(var dbfId in playerCards.Select(x => x.DbfIf).Distinct())
					playerCardsDict[dbfId] = new[] { playerCards.First(c => c.DbfIf == dbfId).Count, FullCount(dbfId) };
			}
